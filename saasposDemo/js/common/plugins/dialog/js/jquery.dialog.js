...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	_init: function( config )
	{
		...
		if( config.icon && !isIfr )
		{
			...
			if(config.icon == 'alert' || config.icon == 'error' || config.icon == 'success'){
				var frontPath = "/front/";
				if(window.Config && window.Config.frontPath){
					frontPath = Config.frontPath;
				}
				
				config.icon = frontPath +"v3/css/img/dialog/" + config.icon + ".gif";
			}
	...
	button: function()
	{
		...
		for( ; i < ags.length; i++ )
		{
			...
			if( item.focus )
			{
			    that._focus && that._focus.addClass(focusButton);
				that._focus = $(button).addClass(focusButton);
				that.focus();
			}
			
			if(item){
				$(button).addClass(focusButton);
			}

...
(function(window, document, undefined)
{
    ...
    window.Pikaday.prototype = {
        ...
        draw: function(force)
        {
            ...
            if (opts.bound) {
                var tableWidth = 0;
                var gridEditCell = false;
                var pEl  = opts.field,
                    left = pEl.offsetLeft,
                    width = pEl.offsetWidth,
                    top  = pEl.offsetTop + pEl.offsetHeight;
                gridEditCell = $(pEl).hasClass("cellEdit");
                while((pEl = pEl.offsetParent)) {
                    left += pEl.offsetLeft;
                    width = pEl.offsetWidth,
                    top  += pEl.offsetTop;
                    if(pEl.nodeName=="TABLE") tableWidth = pEl.offsetWidth;
                }
                if(gridEditCell && left>width && tableWidth!=0){
                    left = left-(left-width)-(tableWidth-left);
                }

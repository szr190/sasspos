...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata){
			...
			result += " title=\""+$.jgrid.stripHtml(tv)+"\"";
			return result + acp[0];
		},
		...
		addCell = function(rowId,cell,pos,irow, srvr, rdata) {
			...
			//设置可编辑状态
			var editableClass = '',cm = ts.p.colModel[pos];
			if(cm.editable === true){
				editableClass = ts.p.editableclass;
			}
			
			return "<td role=\"gridcell\" class=\""+editableClass+"\" "+prp+">"+v+"</td>";
		},
		...
		sortData = function (index, idxcol,reload,sor, obj){
			...
			if(ts.p.multiSort) {
				...
				$("span.ui-grid-ico-sort",previousSelectedTh).removeClass('a');
				$(previousSelectedTh).attr("aria-selected","false");
				if(ts.p.frozenColumns) {
					ts.grid.fhDiv.find("span.ui-grid-ico-sort").addClass('ui-state-disabled');
					ts.grid.fhDiv.find("span.ui-grid-ico-sort").removeClass('a');
					ts.grid.fhDiv.find("th").attr("aria-selected","false");
				}
				$("span.ui-icon-"+ts.p.sortorder,newSelectedTh).removeClass('ui-state-disabled');
				$("span.ui-icon-"+ts.p.sortorder,newSelectedTh).addClass('a');
		...
		$("th",thr).each(function ( j ) {
			...
			if(sort) {$("div span.s-ico",this).show();
				if(ts.p.multiSort) {
					if(ts.p.viewsortcols[0]) {
					...
					$(this.p.colModel).each(function(i){
						...
						if( dval !== undefined) {
							...
							if(t.p.treeGrid===true && nm === t.p.ExpandColumn) {
								...
								if(this.cellattr && $.isFunction(this.cellattr)){
									var acp = "";
									var cssp = "";
									var celp = this.cellattr.call(this, rowid, vl, data, this, null);
									if(celp && typeof celp === "string") {//返回值必须类似于 return "style='color:red'";
										celp = celp.replace(/style/i,'style').replace(/title/i,'title');
										acp = celp.replace('-style','-sti').split(/style/);
										if(acp.length === 2 ) {
											acp[1] =  $.trim(acp[1].replace('-sti','-style').replace("=",""));
											...
											var cssArray = (acp[1].substring(0,acp[1].length-1)).split(",");
											$(cssArray).each(function(j){
												var cssObj = this.split(":");
												$("td[role='gridcell']:eq("+i+")",ind).css(cssObj[0],cssObj[1]);
											});
										} else {
										}
									}
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					for(i = gi+si+ni; i < t.p.colModel.length;i++){
						...
						var editableClass = '';
						if(cm.editable === true){
							editableClass = t.p.editableclass;
						}
						
						row[row.length] = "<td role=\"gridcell\" class=\""+editableClass+"\" "+prp+">"+v+"</td>";
					}
...
//			var tablescrolltop=$("#grid").offset().top;

...
(function($){
	...
	document.writeln('<style>'+
						'.set-time-'+class_num+'{border:solid #ccc 1px;box-shadow: 0 5px 15px -5px rgba(0,0,0,0.5);background:#fff;z-index:2000;overflow:hidden;max-width:260px;_width:530px;}' +
						...
						'.set-time-'+class_num+' .set-time-'+class_num+'-title{padding:5px;background:#fff;overflow:hidden;*zoom:1;}' +
						...
						'.set-time-'+class_num+' .set-time-'+class_num+'-content .set-time-'+class_num+'-content-h{width:70px;}' +
						'.set-time-'+class_num+' .set-time-'+class_num+'-content .set-time-'+class_num+'-content-i{width:170px;}' +
						...
						'.set-time-'+class_num+' td.use:hover{color:#fff;background:#FF8000;} ' +
						'.set-time-'+class_num+' th{padding:2px;background:#fff;border:1px solid #ccc;}' +
						'.set-time-'+class_num+' .now{background:#eee0e0;color:#fff;}' +
						'.set-time-'+class_num+' .default{background:#33AAFF;color:#fff;}' +
						'.set-time-'+class_num+' .selected{background:#33AAFF;color:#fff;}' +
	...
	$.fn.extend({
		setTime : function(options){
			...
			self.attr('readonly','true').bind('click', function(e){
				...
				for(var n = 0, num = 0, num_s = 0; n < 12; n ++){
					html_h += '<tr>';
					for(var n2 = 0; n2 < 2; n2 ++, num ++){
				...
				//秒钟选择框
				var html_s = '<table id="set-time-'+id_num+'-s" style="display:none"><tr><th colspan="10">秒</th>';
...
//				thisHtmlObj.css({display:'block', left:e.pageX-20, top:e.pageY+10});
				thisHtmlObj.css({display:'block', left:selfleft, top:selftop+30});
				showTimeNow($('#set-time-'+id_num+'-now'));

				//关闭
				$('#set-time-'+id_num+'-close').on('click', function(){
					clearData(thisHtmlObj,self)
				});
				$('.set-time-'+class_num+'').on('mouseenter', function(){
				...
				//选择时间
				$('#set-time-'+id_num+'-h td.use').on('click', function(){
				...
				$('#set-time-'+id_num+'-i td.use').on('click', function(){
				...
				$('#set-time-'+id_num+'-s td.use').on('click', function(){
